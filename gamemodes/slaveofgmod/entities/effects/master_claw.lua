...
function EFFECT:Init( data )
	...
	self.Entity:SetModelScale( 20, 0 )
	...
	for i=1, 2 do
		self.Entity:ManipulateBonePosition( bone, Vector( 40, 0, -10) )
	end
	...
	self.Entity:SetPos( data:GetOrigin() + diff - vector_up * 50 )
	
	self.SavePos = data:GetOrigin() + diff - vector_up * 50
	
	/*local bone = self:LookupBone( "ValveBiped.Bip01_L_UpperArm" )
	for i=1, 2 do
		self.Entity:ManipulateBoneAngles( bone, Angle( -40, 10, math.random(-90,90)) )
		self.Entity:ManipulateBonePosition( bone, Vector( 30, 0, 0) )
	end
	...
	self.Entity:SetPos( data:GetOrigin() + diff )
	
	self.SavePos = data:GetOrigin() + diff
	
	local bone = self:LookupBone( "ValveBiped.Bip01_L_Forearm" )
	for i=1, 2 do
		self.Entity:ManipulateBoneAngles( bone, Angle( 0, 45, 0) )
	end	*/
...
function EFFECT:Render()
	...
	self.Entity:ManipulateBonePosition( bone, Vector( -self.StoreX + self.StoreX * delta, -self.StoreY + self.StoreY * delta, -35 + 35 * delta) )	

	if !self.Lowering then
		local bone = self:LookupBone( "ValveBiped.Bip01_L_Hand" )
		self.Entity:ManipulateBoneAngles( bone, Angle( 0, -40 + 90 * delta, 0) )
	...
	self.Entity:ManipulateBoneAngles( bone, Angle( 0, -60 + 100 * delta, 0) )
	...
	self.Entity:ManipulateBoneAngles( bone, Angle( 0, 40 * delta, 0) )
	self.Entity:ManipulateBoneScale( bone, Vector( 1 + 1 * delta, 1 + 1 * delta, 1 + 1 * delta ) )
	
	local bone = self:LookupBone( "ValveBiped.Bip01_L_Finger2" )
	self.Entity:ManipulateBoneAngles( bone, Angle( 0, -80 + 100 * delta, 0) )
	self.Entity:ManipulateBoneScale( bone, Vector( 1 + 0.6 * delta, 1 + 0.6 * delta, 1 + 0.6 * delta ) )
	...
	if self.RefractPos then
		mat:SetFloat("$refractamount", 0.5 * delta)
		mat:SetInt("$ignorez", 0 )
		render.SetMaterial( mat )
		render.UpdateRefractTexture()
		render.DrawQuadEasy( self.RefractPos, vector_up, 200, 200, Color( 255, 255, 255, 255 * delta ), 0 )
	end
