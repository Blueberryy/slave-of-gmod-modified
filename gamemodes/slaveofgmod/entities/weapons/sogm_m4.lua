...
SWEP.Primary.Sound			= Sound("Weapon_M4A1.SingleHeavy")
...
SWEP.AkimboClass 			= "sogm_m4_akimbo"

sound.Add( {
	name = "Weapon_M4A1.SingleHeavy",
	channel = CHAN_WEAPON,
	volume = 1.0,
	level = 100,
	pitch = 117,
...
function SWEP:PrimaryAttack()
	...
	self:EmitFireSound()
	self:TakeAmmo()
	if self.Akimbo and not self.Owner:GetCharTable().AkimboSingleMode then
		self:TakeAmmo()
	end
	...
	if self.OverrideFireBullet then
		self:OverrideFireBullet()
		if self.Akimbo then
			self:OverrideFireBullet()
		end
	else
...
function SWEP:FireBullet()
	...
	if game.SinglePlayer() then
		owner:ShakeView( math.random( self.MinShake or 1, self.MaxShake or 4) ) 
	else
		if CLIENT then
			GAMEMODE:ShakeView( math.random( self.MinShake or 1, self.MaxShake or 4) )
		end
	end
	...
	if SERVER then
		...
		if IsValid(pr) then
			...
			trace.endpos = (owner:GetAttachment(owner:LookupAttachment(att)) and owner:GetAttachment(owner:LookupAttachment(att)).Pos or owner:GetShootPos()) + (self.GetBulletOffset and self:GetBulletOffset(aim) or aim * 9) + predict
			...
			local force = self.BulletSpeed or 2600
